...
with open(f"out/Checkin.abi") as f:
    abi = json.load(f)
    challenge = provider.eth.contract(contract_address, abi=abi)

print("[*] Compile circult")
os.system(f"circom CheckIn.circom --r1cs --wasm")

print("[*] Generate proof")
with open("input.json", 'w') as f:
    json.dump({"a": 1, "b": 2}, f)
os.system(f"node CheckIn_js/generate_witness.js CheckIn_js/CheckIn.wasm input.json witness.wtns")
os.system(f"snarkjs g16p CheckIn_groth16.zkey witness.wtns proof.json public.json")
