...
library MerkleTreeLib {
    ...
    error InvalidTreeDepth(uint256 depth);

    // default zero values for each tree depth
    ...
    function defaultZeroVal(uint256 index) internal pure returns (uint256) {
        ...
        revert InvalidTreeDepth(index);
    }

    function init(MerkleTreeStruct storage tree, uint256 depth) internal {
        if (depth <= 0 || depth > MAX_DEPTH) revert InvalidTreeDepth(depth);

        tree.depth = depth;
        tree.root = defaultZeroVal(depth);
    }

    function insert(MerkleTreeStruct storage tree, uint256 leaf) internal returns (uint256) {
        if (leaf >= SNARK_SCALAR_FIELD) revert InvalidFieldValue(leaf);

        uint256 depth = tree.depth;
        uint256 leafIndex = tree.numberOfLeaves;
        if (leafIndex >= 2 ** depth) revert TreeIsFull();
        ...
        for (uint8 i; i < depth;) {
            ...
            hash = PoseidonT3.hash(tree.lastSubtrees[i]);
            leafIndex >>= 1;
